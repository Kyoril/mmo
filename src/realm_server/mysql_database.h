...
namespace mmo
{
	...
	public:
		...
		void UpdateCharacter(uint64 characterId, uint32 map, const Vector3& position, const Radian& orientation, uint32 level, uint32 xp, uint32 hp, uint32 mana, uint32 rage, uint32 energy, uint32 money, uint32 bankBagSlots, uint32 bindMap, const Vector3& bindPosition, const Radian& bindFacing, const std::vector<uint32>& spellIds, const std::vector<CharacterClassData>& knownClasses, uint32 activeClassId, uint32 timePlayed, const std::vector<uint32>& emoteIds, uint32 moodEmote, uint32 idlePoseEmote, uint32 sitPoseEmote, uint32 sleepPoseEmote) override;
