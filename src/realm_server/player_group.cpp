...
namespace mmo
{
	...
	bool PlayerGroup::CreateFromDatabase()
	{
		...
		m_database.asyncRequestKeyed(m_id, std::move(handler), &IGroupDatabase::LoadGroup, m_id);
	...
	void PlayerGroup::Create(const uint64 leaderGuid, const String& leaderName)
	{
		...
		m_database.asyncRequestKeyed(m_id, std::move(handler), &IGroupDatabase::CreateGroup, m_id, m_leaderGUID, static_cast<uint8>(m_lootMethod), m_lootTreshold);
	}
	...
	void PlayerGroup::SetLootMethod(const LootMethod method, const uint64 lootMaster, const uint32 lootThreshold)
	{
		...
		// Persist to database
		auto handler = [](bool success) {};
		m_database.asyncRequestKeyed(m_id, std::move(handler), &IGroupDatabase::SetGroupLootMethod, m_id, static_cast<uint8>(m_lootMethod), m_lootMaster, static_cast<uint8>(m_lootTreshold));
	}
	...
	void PlayerGroup::SetLeader(const uint64 guid)
	{
		...
		auto handler = [](bool success) {};
		m_database.asyncRequestKeyed(m_id, std::move(handler), &IGroupDatabase::SetGroupLeader, m_id, m_leaderGUID);
	}
	...
	PartyResult PlayerGroup::AddMember(const uint64 memberGuid, const String& memberName)
	{
		...
		// Update database
		auto handler = [](bool success) {};
		m_database.asyncRequestKeyed(m_id, std::move(handler), &IGroupDatabase::AddGroupMember, m_id, memberGuid);
	...
	void PlayerGroup::RemoveMember(const uint64 guid)
	{
		...
		if (it != m_members.end())
		{
			...
			else
			{
				...
				auto handler = [](bool success) {};
				m_database.asyncRequestKeyed(m_id, std::move(handler), &IGroupDatabase::RemoveGroupMember, m_id, guid);
			}
		}
	}
	...
	void PlayerGroup::Disband(const bool silent)
	{
		...
		// Remove from database
		auto handler = [](bool success) {};
		m_database.asyncRequestKeyed(m_id, std::move(handler), &IGroupDatabase::DisbandGroup, m_id);
