...
namespace mmo
{
	...
	PacketParseResult RealmConnector::OnLogonChallenge(auth::Protocol::IncomingPacket& packet)
	{
		...
		if (result == auth::auth_result::Success)
		{
			...
			sendSinglePacket([&](auth::OutgoingPacket& outPacket)
			{
				...
				// payload and misalign the realm server's fixed-size read of A and M1.
				outPacket << io::write_range(m_A.asByteArray(32));
	...
	void RealmConnector::SendCharacterData(uint32 mapId, const InstanceId& instanceId, uint32 timePlayed, const GamePlayerS& character)
	{
