			...
			if (auto strongThis = weakThis.lock())
			{
				...
				strongThis->m_connection->sendSinglePacket([authResult, &strongThis](auth::OutgoingPacket& outPacket) {
					...
					if (authResult == auth::auth_result::Success)
					{
						...
						// Write s as a fixed 32-byte value. The realm reads the salt into a
						// fixed 32-byte buffer, so a variable-length salt (asByteArray()
						// strips leading zero bytes) would break authentication for any
						// realm whose salt happens to have a high zero byte.
