...
namespace mmo
{
	...
	private:
		...
		void DrawShaderCodePanel(const String& panelId);

		/// @brief Captures the high level HLSL generated by the given compiler so it can be inspected
		///	       in the shader code panel.
		void CaptureGeneratedShaderCode(const MaterialCompiler& compiler);

		/// @brief Gets the currently selected generated shader's HLSL source.
		[[nodiscard]] const String& GetSelectedShaderCode() const;
		...
		// Layout management
		void InitializeDockLayout(ImGuiID dockspaceId, const String& previewId, const String& detailsId, const String& graphId, const String& shaderCodeId);
	...
	private:
		...
		std::array<String, 6> m_vertexShaderCode;
		std::array<String, 4> m_pixelShaderCode;
		int m_selectedShaderIndex { 0 };
		bool m_shaderCodeCaptured { false };
	};
}
