...
namespace mmo
{
	...
	ExpressionIndex MaterialCompilerMetal::AddTextureSample(std::string_view texture, ExpressionIndex coordinates, bool srgb, SamplerType type)
	{
		return IndexNone;
	}
	...
	ExpressionIndex MaterialCompilerMetal::AddTime()
	{
		return IndexNone;
	}

	ExpressionIndex MaterialCompilerMetal::AddRotator(ExpressionIndex coordinates, ExpressionIndex center, ExpressionIndex rotation)
	{
		return IndexNone;
	}

	ExpressionIndex MaterialCompilerMetal::AddFresnel(ExpressionIndex exponent, ExpressionIndex baseReflectFraction, ExpressionIndex normal)
	{
		return IndexNone;
	}

	ExpressionIndex MaterialCompilerMetal::AddScreenSpaceReflection(ExpressionIndex worldNormal,
		ExpressionIndex maxDistance, ExpressionIndex stepCount)
	{
		// Stub, like every other expression in this backend: MaterialCompilerMetal generates a
		// placeholder shader and implements none of the expression nodes meaningfully.
		//
		// NOTE: this class is currently abstract regardless. It does not override AddSceneColor,
		// AddSceneDepth, AddPixelDepth, AddScreenPosition, AddSaturate, AddCameraPosition or
		// AddGlobalFunction, all of which are pure virtual on MaterialCompiler, so the macOS
		// client and editor cannot have built since those nodes were introduced. This override
		// exists so the screen-space reflection node does not add to that backlog.
		return IndexNone;
	}
	...
	void MaterialCompilerMetal::GeneratePixelShaderCode(PixelShaderType type)
	{
		m_pixelShaderCode[(int)type] = "// This was generated by MaterialCompilerNull. It doesn't do anything.";
	}

    ExpressionIndex MaterialCompilerMetal::AddTextureParameterSample(std::string_view name, std::string_view texture, ExpressionIndex coordinates, bool srgb, SamplerType type)
    {
        return IndexNone;
    }
