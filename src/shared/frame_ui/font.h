...
namespace mmo
{
	...
	public:
		...
		/// Draws text with hyperlink support, updating hyperlink bounds for hit-testing
		/// @param parsedText The parsed text containing hyperlinks and color changes
		/// @param position The position (in pixels) where to draw the text on screen.
		/// @param buffer The geometry buffer which will receive the generated geometry.
		/// @param scale A scaling factor. Keep in mind that upscaling will result in a loss of quality.
		/// @param color The default color value.
		void DrawTextWithHyperlinks(ParsedText& parsedText, const Point& position, GeometryBuffer& buffer, float scale = 1.0f, argb_t color = 0xFFFFFFFF);
