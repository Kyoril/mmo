...
namespace mmo
{
	namespace proto_client
	{
		...
		struct ProjectLoader
		{
			...
			static bool load(const std::filesystem::path &directory, const Managers &managers, Context &context)
			{
				const std::filesystem::path projectFilePath = "project.txt";
				const auto projectFile = AssetRegistry::OpenFile((directory / projectFilePath).string());
				if (!projectFile)
				{
				...
				for (const auto &manager : managers)
				{
					...
					const auto managerFile = AssetRegistry::OpenFile((directory / relativeFileName).string());
					if (!managerFile)
					{
						success = false;
